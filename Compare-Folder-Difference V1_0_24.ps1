...
# === Ask user for comparison type before scanning ===
...
# === Actual Comparison with Progress Bar ===
...
foreach ($relPath in $sourceMap.Keys) {
    ...
    if ($tgt) {
        ...
        if (-not $isDiff -and $useExtended) {
            Write-Progress -Activity "Comparing file hashes..." -Status "$relPath" -PercentComplete (($currentIndex / $totalToCompare) * 100)
            try {
